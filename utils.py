...
def convert_dotbracket_to_matrix(s, allow_pseudoknots=False):
    ...
    bp_list = convert_dotbracket_to_bp_dict(s, allow_pseudoknots=allow_pseudoknots)
...
def load_package_locations(DEBUG=False):
    ...
    if "e2efold" in return_dct and "e2efold_conda_env" not in return_dct:
...
def compare_structure_to_native(s, native, metric="all", PK_involved=None):
    if metric not in ["PPV", "sensitivity", "F1_score", "all"]:
        raise ValueError('Only PPV, sensitivity, F1_score and all comparison currently implemented.')

    if PK_involved is None:
        s_list = convert_dotbracket_to_bp_list(s, allow_pseudoknots=True)
        native_list = convert_dotbracket_to_bp_list(native, allow_pseudoknots=True)
    elif PK_involved:
        s_list = _seperate_structure_into_PK_involved_or_not(s)["pk_bps"]
        native_list = _seperate_structure_into_PK_involved_or_not(native)["pk_bps"]
    else:
        s_list = _seperate_structure_into_PK_involved_or_not(s)["no_pk_bps"]
        native_list = _seperate_structure_into_PK_involved_or_not(native)["no_pk_bps"]
...
def compare_structures_to_natives(structs, natives, comparison="basepairs", metric="all"):
    ...
    if comparison not in ["basepairs", "is_PK", "non_PK_basepairs", "PK_basepairs"]:
        raise ValueError('Only basepairs and is_PK comparison currently implemented.')
    if metric not in ["PPV", "sensitivity", "F1_score", "all"]:
        raise ValueError('Only PPV, sensitivity, F1_score and all comparison currently implemented.')

    if comparison == "basepairs":
        structs_list = [convert_dotbracket_to_bp_list(s, allow_pseudoknots=True) for s in structs]
        natives_list = [convert_dotbracket_to_bp_list(native, allow_pseudoknots=True) for native in natives]

        PP = sum(len(x) for x in structs_list)
        P = sum(len(x) for x in natives_list)

        TP = sum(len([z for z in x if z in y]) for x, y in zip(structs_list, natives_list))  # true positives

    elif comparison == "non_PK_basepairs":
        structs_list = [_seperate_structure_into_PK_involved_or_not(s)["no_pk_bps"] for s in structs]
        natives_list = [_seperate_structure_into_PK_involved_or_not(native)["no_pk_bps"] for native in natives]
        PP = sum(len(x) for x in structs_list)
        P = sum(len(x) for x in natives_list)
        ...
        structs_list = [_seperate_structure_into_PK_involved_or_not(s)["pk_bps"] for s in structs]
        natives_list = [_seperate_structure_into_PK_involved_or_not(native)["pk_bps"] for native in natives]
        PP = sum(len(x) for x in structs_list)
        P = sum(len(x) for x in natives_list)
        ...
        TP = sum([x and y for x, y in zip(s_is_PK, native_is_PK)])  # true positives
        FP = sum([x and not y for x, y in zip(s_is_PK, native_is_PK)])  # false positives
        FN = sum([not x and y for x, y in zip(s_is_PK, native_is_PK)])  # false negatives
        TN = sum([not x and not y for x, y in zip(s_is_PK, native_is_PK)])  # true negatives
